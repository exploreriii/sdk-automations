...
There is no `types/`, `utils/`, `helpers/` or `lib/` — **naming by kind forces you to already know
the answer in order to find it**, which is backwards for a newcomer.

The same test applies to a file's name, and two names in one directory must not mean different
things: a lookup named after the config section it had nothing to do with, while that section's
reader lived elsewhere under almost the same name, cost real confusion (D103).

A directory documents itself in its `index.ts` header — what it owns, what it does not, its rules and
traps. There are no READMEs below a package root.
...
**Follow the question, not the noun.** A thing belongs where its *question* is asked, often not where
its subject matter sits: the reverse label lookup lives in `config/` rather than the engine, because
the validator's collision check and the lookup must fold strings identically (D55).

**Every arrow points one way, and one directory is the root.** In `core/`, `config/` imports nothing.
A new import into the root means something is in the wrong place.

**A type-only cycle is still a cycle** (D91, D92) — "it is erased at runtime" is not a defence.

**A bridge lives at one end.** When a table's keys come from one concern and its values from another,
pick an end, state why, and stop.

## Depth and graduation

**A target earns a subdirectory only when it needs a second file** (D89) — against premature nesting;
it does not govern splitting files.
...
**Structure follows substance only as far as substance leads.** When a reorganisation stops paying,
stop and write down what you declined and why — D92 phase 5 executed the subset that was real. A
rejected move with a recorded reason is worth more than a completed one nobody can question.
...
- **Nothing added or lost** — the sorted, comment-stripped declarations must match:
  ...
  ```
- **`.gitignore` rules move in the same breath as the directory** (D95: credentials were briefly
  stageable because the rules still named the old paths).
