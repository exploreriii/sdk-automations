...
## The pillar catalogue

Closed, like every platform vocabulary — each type is a counting resolver the platform provides,
and a new type enters by review:
...
| `observations` | a contributor-sweep entry (new — sibling of `staleItemsDue`, carrying per-contributor pillar counts the platform computed) |
...
| `minTier` pillar without the `skills` mappings | reported as unusable, not silently ignored |
...
| `noticeOn: trackingIssue` with `noticeIssue` unset | reported as unusable, not silently ignored |
