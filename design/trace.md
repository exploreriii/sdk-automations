...
6. **If you need a word the platform lacks**, this is what each costs and where it lives. A GitHub
   ...
   |---|---|---|
   | an operation | one module in each of `core/src/intents/operations/`, `runtime/src/shell/apply/operations/`, `runtime/src/adapter/operations/` + a key in `IntentCatalogue` | a payload pin in the shell's effects test; `pnpm contracts`; `design/guides/write-operations.md` |
