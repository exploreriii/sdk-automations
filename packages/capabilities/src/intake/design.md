...
|---|---|---|
| 1 | onOpen label + welcome, approval confirm | nothing new — issue facts, `applyMappedLabel`, `postManagedComment` all exist; the closest-to-buildable capability in the pool |
...
| `facts` / `needs` | `issue`, needing no group (exists) — needs adding: lock state (phase 2), native issue type (phase 3) |
