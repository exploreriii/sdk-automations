...
| The three sweep reads `design/guides/sweep.md` §1 marks unconfirmed — the review decision, the timeline on a pull request number, the commits page | `facts.ts` `CONFIRMED_SWEEP_READS` | nothing yet | **unprobed** | a sandbox protocol run cites them | none — the `review` group answers `unread` and the pull-request ladder stays silent until then |
