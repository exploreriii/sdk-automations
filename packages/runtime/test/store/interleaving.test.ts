...
/** Three hundred store steps on a shared CI runner, the first of them opening the file cold. */
...
describe("claims under random interleaving: store ≡ single-holder lease model", () => {
    it.each(SEEDS)("seed %i — 300 steps, two instances, three effects", (seed) => {
        withTempDir("store-stress-", (dir) => {
            const path = join(dir, "store.sqlite");
            const a = new Store(path);
            const b = new Store(path);
            // withTempDir owns the directory; the two handles are still ours,
            // and closing them before the removal is what keeps this clean.
            try {
...
describe("schedules under random interleaving: fire exactly once unless requeued", () => {
    it.each(SEEDS)(
        "seed %i — 200 steps, two instances, eight schedules",
        (seed) => {
            withTempDir("store-stress-", (dir) => {
                const path = join(dir, "store.sqlite");
                const a = new Store(path);
