 ...
 * the standing rule in `CONTRIBUTING.md`. Each suite keeps the
...
const STORE_SIBLINGS = ["instants", "schema", "fold", "ledger"] as const;
...
export function buildWorkerStoreModule(directory: string): string {
    ...
    writeFileSync(
        storeModule,
        transpile(storeSource).replace("@hiero-hackers/automation-core", "./ids.js"),
    );
